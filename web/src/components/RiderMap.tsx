...
export default function RiderMap({ onRouteSelected }: RiderMapProps) {
    ...
    const handleMapClick = async (e: L.LeafletMouseEvent) => {
        ...
        debounceTimeoutRef.current = setTimeout(async () => {
            ...
            // Leaflet Polyline expects [lat, lng] per point
            const coords = data.route?.geometry?.[0]?.coordinates;
            const parsedRoute = coords?.map((coord: { latitude: number; longitude: number }) => [coord.latitude, coord.longitude] as [number, number]) ?? []
        ...
        <div className="relative flex flex-col md:flex-row h-screen">
            <div className={`${destination ? 'flex-[0.7]' : 'flex-1'}`}>
                ...
                >
                    ...
                    {drivers?.filter((d) => d?.location?.latitude != null && d?.location?.longitude != null).map((driver) => (
                        <Marker
                            key={driver?.id}
