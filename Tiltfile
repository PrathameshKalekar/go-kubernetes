...
# Use arm64 for Apple Silicon, amd64 for Intel Mac/Linux
go_arch = 'arm64'  # change to 'amd64' if on Intel
...
gateway_compile_cmd = 'CGO_ENABLED=0 GOOS=linux GOARCH=' + go_arch + ' go build -o build/api-gateway ./services/api-gateway'
if os.name == 'nt':
...
trip_compile_cmd = 'CGO_ENABLED=0 GOOS=linux GOARCH=' + go_arch + ' go build -o build/trip-service ./services/trip-service/cmd/main.go'
if os.name == 'nt':
...
driver_compile_cmd = 'CGO_ENABLED=0 GOOS=linux GOARCH=' + go_arch + ' go build -o build/driver-service ./services/driver-service'
if os.name == 'nt':
...
docker_build(
  ...
  build_args={
    ...
    'NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY': os.getenv('NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY', 'pk_test_51T0y710QknN9e6Xz88OH2o4WqAJoUubQVB6CV5Mr38GwgZjINlpIdh3GDsyDiLjtKB7a4kj8DQFXnoyoMtfiT5DU00qMWGHtMn'),
  },
)
...
payment_compile_cmd = 'CGO_ENABLED=0 GOOS=linux GOARCH=' + go_arch + ' go build -o build/payment-service ./services/payment-service/cmd/main.go'
if os.name == 'nt':
